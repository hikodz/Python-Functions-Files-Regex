...
class CountDownTimer:
    def __init__(self):
        ...
        self.time_entry = tk.Entry(self.root,font=("Helvetika",30))
        self.time_entry.grid(row=0, column=0, columnspan=2, padx=5, pady=5)
        

        self.start_button = tk.Button(self.root, font=("Helvetika",30), text="Start", command=self.start_thread)
        self.start_button.grid(row=1, column=0, padx=5, pady=5)

        self.stop_button = tk.Button(self.root, font=("Helvetika",30), text="Stop", command=self.stop)
        self.stop_button.grid(row=1, column=1, padx=5, pady=5)

        self.time_label = tk.Label(self.root, font=("Helvetika",30), text= "Time: 00:00:00")
    ...
    def start(self):
        ...
        while full_seconds >0 and not self.stop_loop:
            ...
            self.time_label.config(text=f"Text: {hours:02d}:{minutes:02d}:{seconds:02d}")    
            self.root.update()
            time.sleep(1)
        #if not self.stop_loop:
        #    toast = ToastNotifier()
        #    toast.show_toast("CountDown Timer", "Time is up!", duration=5)
